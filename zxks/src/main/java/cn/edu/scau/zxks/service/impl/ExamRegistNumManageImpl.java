...
public class ExamRegistNumManageImpl implements ExamRegistNumManage 
{
	...
	public String addGoAbroadMessage(Study_abroad studyAbroad)  //添加留学生信息
	{
		try
		{
			Student student = studentMapper.selectByPrimaryKey(studyAbroad.getStuNum());
			if (student.getGraduateState() == 5||student.getGraduateState() == 6)
			{
	...
	public int judgeIsBeforeGraduate(String studentId)
	{
		try
		{
			Student student = studentMapper.selectByPrimaryKey(studentId);
