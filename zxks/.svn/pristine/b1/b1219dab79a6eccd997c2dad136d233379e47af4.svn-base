...
<body>
                            ...
                            <div class="col-md-12">
                                <form class="form-horizontal" role="form">
                                    <div class="form-group">
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control" id="inputvalue" placeholder="输入科目、考试地点、有关咨询电话等多种查询条件">
                                        </div>
                                        <div class="col-sm-4">
                                                    ...
                                                    <c:forEach items="${ examList}" var="e">
                                                        <tr>
                                                            ...
                                                            <c:forEach items="${ majorList}" var="major">
                                                                <c:if test="${major.majorId == e.majorId}">
                                                                    <td>${major.majorName }</td>
                                                                </c:if>
                                                            </c:forEach>
                                                            <td>${e.examName }</td>
                                                            <td><fmt:formatDate value="${e.applyStart }" pattern="yyyy-MM-dd"/></td>
                                                            <td><fmt:formatDate value="${e.applyEnd }" pattern="yyyy-MM-dd"/></td>
                                                            <td><fmt:formatDate value="${e.time }" pattern="yyyy-MM-dd"/></td>
                                                            <td>
                                                                <a href="<%=path %>/apply/changeExamMsg?examId=${e.examId }" class="btn btn-primary" style="padding:0px 33px;">设置</a>
                                                            </td>
                                                        </tr>
                                                   </c:forEach>
                                                </tbody>
    ...
    var trVal = $(".r-down-footing .table tbody tr");
    ...
    // 点击查询
    $(".r-down-footing form .form-group .btn-search").click(function(event) {
        // 刷新
        // alert(tdLength);
        refresh++;
        if (refresh == 1) {
        ...
        // 获取搜索框的值
        var inputValue = $(".r-down-footing form .form-group #inputvalue").val();
        console.log("输入的值：" + inputValue);
        // 点击查询后，重置搜索框的值
        $(".r-down-footing form .form-group #inputvalue").prop('value', '');
        // var tdVal = 0;
        var treq = 0;
        // 判断搜索是否在同一行
        var panduan = 0;
